...
function addProject(txt) {
    function warn_limit_project() {
        ...
        if (ul_projects.children.length >= 6) {
            ...
            const txt = create.simpleElement(
                "h3",
                "",
                "Limit of projects are 5."
            );
            const btnClose = create.btnCreator([], "Close");
    ...
    const div = create.simpleElement("div", "", txt);
...
function createTaskDom(elChildren) {
    ...
    const card = create.simpleElement("div", "card-" + all_el["storageKey"]);
    ...
    els_info.forEach((item) => {
        const el = create.simpleElement(item[0], item[1], item[2]);
        card.appendChild(el);
    });
    ...
    function btnStuffs() {
        ...
        const btn = create.simpleElement("div", "btn-card-container");
